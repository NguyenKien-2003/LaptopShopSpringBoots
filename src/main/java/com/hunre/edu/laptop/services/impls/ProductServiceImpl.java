...
import com.hunre.edu.laptop.models.*;
import com.hunre.edu.laptop.repositories.*;
import com.hunre.edu.laptop.services.FileUploadService;
import com.hunre.edu.laptop.services.ProductService;
import com.hunre.edu.laptop.utils.QrCodeGenerator;
...
public class ProductServiceImpl implements ProductService {
    ...
    @Override
    public Page<ProductVariantDetailsResponse> getFilteredProducts(int minPrice, int maxPrice, List<Long> brandIds,List<Long> categoryIds, Pageable pageable) {
        Page<ProductVariant> productVariants = null;
        if (brandIds.size() != 0 && categoryIds.size() != 0) {
            productVariants = productVariantRepository.findByPriceRangeAndBrandsAndCategory(minPrice, maxPrice, brandIds,categoryIds, pageable);
        } else if (brandIds.size() !=0 && categoryIds.size() == 0) {
            productVariants = productVariantRepository.findByPriceRangeAndBrands(minPrice, maxPrice, brandIds, pageable);
        } else if (categoryIds.size() != 0 && brandIds.size() == 0) {
            productVariants = productVariantRepository.findByPriceRangeAndCategories(minPrice, maxPrice, categoryIds, pageable);
        } else {
...
//        Pageable pageable = PageRequest.of(0, limit);
//
//        return productVariantRepository.findProductsByCategory(categoryId, pageable)
//                .getContent()
//                .stream()
//                .map(this::getDefaultVariantFromVariant)
//                .distinct()
